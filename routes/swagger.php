...
// Check if $swagger is not empty
if (!empty($swagger)) {
    echo json_encode($swagger, JSON_PRETTY_PRINT);
} else {
    // Handle the case where $swagger is empty
    http_response_code(500);
    echo json_encode([
        "error" => "Swagger definition is empty",
        "message" => "The Swagger documentation could not be generated."
    ]);
}